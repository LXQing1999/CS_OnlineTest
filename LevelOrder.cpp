...
void LevelOrder(TreeNode *root)
{
    ...
    while (!pos.empty())
    {
        TreeNode *current = pos.front();
        pos.pop();
        printf("%c", current->data);

        if (current->leftChild != NULL)
        {
            pos.push(current->leftChild);
        }
        if (current->rightChild != NULL)
        {
            pos.push(current->rightChild);
        }
    }
    printf("\n");
}
...
{ // 引用的根节点和队列都涉及了修改，所以要加&
    if (data != '#')
    {
        ...
        // myQueue.push(pQueueNode); // 你这个观看时长又没有拉到最后，他自己最后编译肯定也会出问题，你自己没耐心看到最后，急吼吼打完了，编译报错
        //  然后这个地方，push操作本来就是压入一个变量，不管是内置类型（你知道啥是内置类型吗）别TM动了
